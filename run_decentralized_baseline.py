#!/usr/bin/env python3
"""
run_decentralized_baseline.py
============================
Decentralized baseline test (no BiasNet, no FusionNet) with STRICT parameters
for comparison against the enhanced ML versions.
"""
...
def main():
    """Run complete decentralized test with baseline (no ML models)."""
    ...
    # Remove old output
    subprocess.run(["rm", "-rf", "outputs_swarm/baseline"], capture_output=True)
    
    print("=== Starting Decentralized Baseline Test (STRICT, no ML) ===")
    ...
    logger_cmd = [
        ...
        "--fanout",
        "--planner", "none",
        "--out", "outputs_swarm/baseline"
    ]
    # Note: No --fusionnet_dir for baseline
    
    print("Starting logger (baseline gossip CI)...")
    logger_proc = run_command_background(logger_cmd, "logger_baseline.log")
    time.sleep(3)
    ...
    node1_proc = run_command_background(node1_cmd, "node1_baseline.log")
    time.sleep(2)
    ...
    node2_proc = run_command_background(node2_cmd, "node2_baseline.log")
    ...
    # Check results
    results_dir = "outputs_swarm/baseline/default_3_random_0_ifo003"
    ...
    # Show log excerpts
    print("\n=== Log outputs ===")
    for log_name in ["logger_baseline.log", "node1_baseline.log", "node2_baseline.log"]:
